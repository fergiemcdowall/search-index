...
test('can concurrently PUT', async function (t) {
  ...
  const docs = new Array(size).fill().map(
    (doc, i) => ({ text: 'this is some test text', _id: i })
  )
  ...
  t.deepEquals(
    await Promise.all(
      docs.map(doc => PUT([doc]))
    ), new Array(size).fill().map(
      (res, i) => [{ _id: i + '', operation: 'PUT', status: 'CREATED' }]
    )
  )
