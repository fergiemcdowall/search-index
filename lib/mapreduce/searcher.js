          ...
          if (++counter < fks.length) {getFacetSets(facetKeySet, counter);}
            ...
            for (k in facetsSoFar) {
              ...
              if (q.facetSort) {
                ...
                finalFacets[k].sort(sortOnValueAsc);
              }
...
//      console.log('facets: ' + JSON.stringify(facets))
      ...
      console.log(RIKeySet);

      indexesMultiply.get(RIKeySet, function(err, data) {
        //console.log(data);
        //Dump all hits with all terms in unsorted object
        var unsortedHits = {};
        for (var k in data) {
          //account for weighting
          var fieldWeight = 1;
          var fieldName = k.split('~')[1];
          if (q.weight)
          ...
          var seekCutOff = 50000;

          //if there is only one key in the set, and only one query term
          //set cutoff to be the same length as the page
          if ((q.pageSize) && (RIKeySet.length == 1)) seekCutOff = q.pageSize;

          for (var i = 0; ((i < data[k].length) && (i < seekCutOff)); i++) {
            if (frequencies.allDocsIDsInResultSet.indexOf(data[k][i][1]) != -1) {
              var tuple = [k, (fieldWeight*data[k][i][0])];
              try {
                unsortedHits[data[k][i][1]].push(tuple);
              }
              catch (err) {
                unsortedHits[data[k][i][1]] = [tuple];
              }
            }
          }
        }
