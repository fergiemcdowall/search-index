...
exports.search = function (reverseIndex, reverseIndexMultiply, docFreqIndex, q, callback) {
...
//    console.log(searchType);
    ...
    if (searchType == 'SINGLE-KEY-SEARCH') {
      ...
      bloomSearch.search(reverseIndex, reverseIndexMultiply, keySet, docFreqs, q, cleanQuery, filterKeySet, totalDocs, function(msg) {
        callback(msg);
      });
    }
    else {
  ...
  var docFreq = function (keySet, i) {
    docFreqIndex.get(keySet[i], function (err, value) {
      ...
      if (err) {
        if (err.notFound) {
          docFreqs[keySet[i]] = 0;
          console.log('KEY ' + keySet[i] + ' NOT FOUND IN DICTIONARY');
        }
      }
