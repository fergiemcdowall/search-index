...
//    logger.debug(searchType);
    ...
    if (searchType == 'SINGLE-KEY-SEARCH') {
      ...
      bloomSearch.search(indexes, indexesMultiply, keySet, docFreqs, q, cleanQuery, filterKeySet, totalDocs, function(msg) {
        callback(msg);
      });
    }
    else {
...
//  logger.debug('filterKeySet: ' + filterKeySet);
  ...
  var docFreq = function (keySet, i) {
    indexes.get(keySet[i], function (err, value) {
      docFreqs[keySet[i]] = value;
      if (err) {
        if (err.notFound) {
          docFreqs[keySet[i]] = 0;
          logger.debug('KEY ' + keySet[i] + ' NOT FOUND IN DICTIONARY');
        }
      }
