...
exports.search = function (reverseIndex, docFreqs, q, cleanQuery, callback) {
  ...
  console.info('got key for ' + leastFrequentKey);
...
//      console.log(data.key);
      ...
      if (bloomFilterTrue) {
        ...
        vectorsRequested++;
        console.log(keyFields[6] + ' ' + vectorsRequested);
        reverseIndex.get('VECTOR~forage.composite~' + keyFields[6] + '~', function (err, value){
          vectorsProcessed++;
          console.log(data.key + ' ' + vectorsRequested + ' ' + vectorsProcessed);
          var hit = {};
          for (var i = 0; i < otherKeys.length; i++) {
            if (value[otherKeys[i]]) bloomValidated = true;
            else bloomValidated = false;
          }
          if (bloomValidated) {
            hit.id = keyFields[6];
            hit.vector = value;
...
*/
      ...
      lookUpFinished = true;
              console.log(lookUpFinished + ' '
                            ...
                            + vectorsProcessed + ' '
                            + queueWorkersFinished + ' '
                            + queueWorkersPushed + ' lookup finished');
