...
exports.search = function (reverseIndex, keySet, docFreqs, q, cleanQuery, filterKeySet, totalDocsInIndex, callback) {
  var hits = [];
  var totalHits = 0;
  ...
  var seekLimit = -1;
  debugger;
  if (Object.keys(docFreqs).length == 1 && !q['facets']) {
    seekLimit = q['pageSize'] + q['offset']
    totalHits = docFreqs[leastFrequentKey];
  }
...
//        console.log('testing bloom for ' + otherKeys[i] + ' is ' + bloom.test(otherKeys[i]));
      ...
      if (bloomFilterTrue && filterSatisfied) {
        var getVectorField = function (counter, hit, id) {
          ...
          hit.tf[field] = {};
          var vectorKey = 'VECTOR~' + field + '~' + id + '~';
          vectorsRequested++;
          reverseIndex.get(vectorKey, function (err, val){
            if (err) {vectorsProcessed++; return;}
            var termIsInVector = [];
            for (var i = 0; i < cleanQuery.query[field].length; i++) {
              var term = cleanQuery.query[field][i];
              if (val.vector[term]) {
