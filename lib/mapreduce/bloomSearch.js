...
exports.search = function (reverseIndex, docFreqs, q, cleanQuery, filterKeySet, callback) {
  ...
  var queue = async.queue(function (hit, callback) {
    ...
    var tf = {};
    hit.score = 0;
    for (var k in cleanQuery.query) {
      for (var i = 0; i < cleanQuery.query[k].length; i++) {
        tf[cleanQuery.query[k][i]] =
          hit.vector[cleanQuery.query[k][i]];
        hit.score += parseFloat(tf[cleanQuery.query[k][i]]);
      }
    }
    delete hit.vector;
    hit.relevance = {};
