...
exports.search = function (indexesMultiply, q, callback) {
  ...
  getDocumentFreqencies(indexesMultiply, q, keySet, function(err, frequencies) {
    ...
    async.parallel([
      ...
      function(callback) {
        getFacets(q, frequencies, indexesMultiply, function(facets){
...
var getDocumentFreqencies = function(indexesMultiply, q, keySet, callback) {
  ...
  async.map(keySet, function(item, callbacky){
    var uniq = [];
    indexesMultiply.createReadStream({'gte':item[0], 'lte':item[1]})
      .on('data', function (data) {
...
var getResults = function(q, frequencies, indexesMultiply, callbackX) {
  var hits = [];
  ...
  async.mapSeries(frequencies.docFreqs, function(item, callbacker) {
    ...
    var seekCounter = 0;
    indexesMultiply.createReadStream({'gte':gte, 'lte':lte})
      .on('data', function (data) {
...
//    console.log(result);
    for (var i = 0; i < result[0].tf.length; i++) {
      var hit = {};
      hit['id'] = result[0].tf[i][1];
      hit.relevance = [];
      hit.relevance.push([result[0].field, result[0].token, result[0].tf[i][0]]);

      //TODO: add field weighting
      for (var j = 1; j < result.length; j++) 
        for (var k = 0; k < result[j].tf.length; k++) 
          if (result[j].tf[k][1] == hit.id)
            hit.relevance.push([result[j].field, result[j].token, result[j].tf[k][0]]);
      hit['score'] = 0;
      for (var j = 0; j < hit.relevance.length; j++)
        hit.score = +hit.score + +hit.relevance[j][2];
      hits.push(hit);
    }


//TODO: add weighting here. A weighted set will always be a subset of a 
// * set, so no intersection necessary
    ...
    async.mapSeries(hits, function(item, callback) {
      indexesMultiply.get('DOCUMENT~' + item.id + '~', function(err, value) {
        ...
        if (q.teaser && item.document[q.teaser]) {
          ...
          } catch (e) {searchIndexLogger.error('error with teaser generation: ' + e)}
        }
        callback(null, item);
      })
    }, function(err, result) {
