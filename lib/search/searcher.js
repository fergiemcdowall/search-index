...
var getNonRangedFacet = function(totalQueryTokens, index, facetRangeKeys, callbacky) {
  async.reduce(facetRangeKeys, {}, function(memo, item, callback){
    ...
    index.createReadStream({gte:item.start, lte:item.end})
      .on('data', function (data) {
        memo[facetName].push({key: data.key.split('~')[4], value:uniqFast(data.value)})
      })
      .on('end', function () {
        callback(null, memo);
      })
    ...
    for (facetRange in result) {
      result[facetRange].sort(sortOnKeyAsc).reduce(function(a, b, i, arr){
        if (a.key == b.key) {
      ...
      if (result[facetRange][result[facetRange].length -1].counter < totalQueryTokens)
        delete result[facetRange][result[facetRange].length -1];
      else if (result[facetRange][result[facetRange].length -1].counter == totalQueryTokens)
        delete result[facetRange][result[facetRange].length -1].counter;
    ...
    if (item.ranges) {
      for (var i = 0; i < item.ranges.length; i++) {
        for (var sfqt in searchFieldQueryTokens) {
          ...
          range.start = prefix + item.ranges[i][0];
          range.end = prefix + item.ranges[i][1] + '~';
          facetRangeKeys.push(range);
        }
      } 
      getBucketedFacet(index, facetRangeKeys, function(facet){
        callback(null, {'key': facetName, 'value': facet});
      })
    }
    else {
      var ranges = q.facetRanges[facetName].ranges || [['','~']];
...
//  var rangedFacets = lo.pluck(lo.filter(q.facetRanges, function(item){return item.ranges}));
//  var unrangedFacets = lo.pluck(lo.filter(q.facetRanges, function(item){return item.ranges === undefined}));

//  console.log('ranged: ' + rangedFacets);
//  console.log('unranged: ' + unrangedFacets);



/*
  var isRanged = (q.facetRanges[facetName].ranges) ? true:false
  if (isRanged) {
    getBucketedFacet(index, facetRangeKeys, function(facet){
      callbacky(facet);
    })
