  ...
  if (isRanged) {
    async.reduce(facetRangeKeys, {}, function(memo, item, callback){
      ...
      index.createReadStream({gte:item.start, lte:item.end})
        ...
        .on('end', function () {
          if (isRanged) memo[facetName].push({"key":key, "value": uniqFast(thisSet).sort()});
          callback(null, memo);
        })
    }, function(err, result){
      ...
      for (facetRange in result) {
        result[facetRange].sort(sortOnKeyAsc).reduce(function(a, b, i, arr){
          if (a.key == b.key) {
            ...
            if (a.counter < searchFieldQueryTokens.length) delete arr[i - 1];
        ...
        if (result[facetRange][result[facetRange].length -1].counter < searchFieldQueryTokens.length)
          delete result[facetRange][result[facetRange].length -1];
        else if (result[facetRange][result[facetRange].length -1].counter == searchFieldQueryTokens.length)
