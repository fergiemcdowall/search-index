...
//      console.log(frequencies)
  ...
  var getDocumentFreqencies = function (indexes, q, keySet, callback) {
    ...
    async.map(keySet, function (item, callbacky) {
      ...
      var df = results.reduce(function(prev, cur, i, arr) {
      ...
      var intersection = _.values(df).reduce(function(prev, cur, i, arr) {
        return _.intersection(prev, cur);
      })
      for (i in df) df[i] = df[i].length;
      var docFreqs = [];
      ...
      for (var i = 0; i < result[0].tf.length; i++) {
        ...
        result.forEach(function(tokenSets) {
          tokenSets.tf.forEach(function(tokenSet){
            ...
            var weight = 1;
            if (q.weight) if (q.weight[tokenSets.field])
              weight = q.weight[tokenSets.field];
            hit.relevance.push(
              [tokenSets.field,
               tokenSets.token,
               tokenSet[0],
        ...
        hit.relevance = _.uniq(hit.relevance, function (n) {
          return n[0] + n[1];
        });
        hit.relevance.forEach(function(tfEntry){
          //tf-idf here
          var tf = +tfEntry[2];
          var weight = +tfEntry[3];
          var totalDocsInIndex = +frequencies.totalDocsInIndex;
          var df = +frequencies.df[tfEntry[0] + '￮' + tfEntry[1]];
          var idf = weight * Math.log10(totalDocsInIndex / df);
