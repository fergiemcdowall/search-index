...
module.exports = function (options) {
  ...
  function indexDoc (reverseIndex, doc, batchOptions, callback) {
    ...
    docToBeIndexed['*'] = createCompositeField(docToBeIndexed, batchOptions);
    ...
    for (var j in searchableFields) {
      ...
      if (!docToBeIndexed.hasOwnProperty(fieldKey)) continue;
      ...
      var docVector = [];
      for (var i = 0; i < docToBeIndexed[fieldKey].length; i++) {
        docVector[i] = {};
        try {
          docVector[i]['field'] = tv.getVector(docToBeIndexed[fieldKey][i].field + '', {
            separator:/[\|' \.,\-|(\n)]+/,
            stopwords: options.stopwords,
            nGramLength: batchOptions.nGramLength
          });
          docVector[i]['weight'] = docToBeIndexed[fieldKey][i].weight;
        }
        catch (e) {
...
//      console.log(JSON.stringify(docVector, null, 2))
      ...
      fieldBatch.push({
        type: 'put',
        key: 'DELETE-DOCUMENT~' + docToBeIndexed.id[0].field + '~' + fieldKey,
        value: deleteKeys
      });
    }
