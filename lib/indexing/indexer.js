...
function indexDoc(reverseIndex, docID, doc, facets, callback) {
  ...
  for (fieldKey in doc) {
    ...
    var docVector = [];
    try {docVector = tv.getVector(doc[fieldKey] + '',
                                  {separator:/[\|' \.,\-|(\n)]+/});}
    catch(e) {}
    //wildcard char
    docVector.push(['*', 1]);
        ...
        for (var j = 0; j < doc[facets[i]].length; j++) {
          for (var k = 0; k < docVector.length; k++) {
            var bloomEntry = fieldKey + '~' + docVector[k][0] + '~' +
              facets[i] + '~' + doc[facets[i]][j];
            tfValue = (docVector[k][1] / highestFrequencyCount / docVector.length).toFixed(10);
    ...
    //no facets
    for (var k = 0; k < docVector.length; k++) {
      var entrykey = fieldKey + '~' + docVector[k][0] + '~~';
      tfValue = (docVector[k][1] / highestFrequencyCount / docVector.length).toFixed(10);
