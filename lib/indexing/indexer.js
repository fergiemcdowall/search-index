...
module.exports = function (options) {
  ...
  var createCompositeField = function(doc) {
    var compositeField = "";
    for (fieldKey in doc) {
      if (_.isArray(doc[fieldKey])){
  ...
  var getCanDoFieldedSearchOn = function(doc, canDoFieldedSearchOn) {
    if (!options.fieldedSearchOnAllFieldsByDefault) {
      for (fieldKey in doc) {
        if ((canDoFieldedSearchOn.indexOf(fieldKey) == -1) && (fieldKey != 'id'))
          delete doc[fieldKey];
      }
    }
  ...
  function indexDoc (reverseIndex, doc, batchOptions, callback) {
    ...
    doc['*'] = createCompositeField(doc);
    var canDoFieldedSearchOn = getCanDoFieldedSearchOn(doc, batchOptions.canDoFieldedSearchOn);
    var tfValues = [];
