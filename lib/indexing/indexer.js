  ...
  for (fieldKey in doc) {
    ...
    try {
      docVector = tv.getVector(doc[fieldKey] + '', {
        separator:/[\|' \.,\-|(\n)]+/,
        stopwords: indexOptions.stopwords
      });
    }
    catch (e) {}
    //wildcard char
...
exports.addDocToIndex = function (indexes, batch, options, callbacky) {
  ...
  deleter.deleteBatch(deleteBatch, indexes, function (err) {
    ...
    async.eachSeries(batch, function (item, callback) {
      ...
      indexOptions.log.info('indexing ' + item.id);
      indexDoc(indexes, item.id, item, options.filters, function (err, tfValues) {
        tf[item.id] = tfValues;
        return callback(err);
      });
    }, function (err) {
      ...
      if (Object.keys(tf).length > 0) {
        calibrater.incrementallyCalibrate(indexes, reduceTF(tf), function (err) {
          indexOptions.log.info('indexed ' + options.batchName);
