...
function insertIntoOrderedArray(element, array) {
  array.splice(locationOfElementInArray(element, array) + 1, 0, element);
  return array;
}

function locationOfElementInArray(element, array, start, end) {
  start = start || 0;
  end = end || array.length;
  var pivot = parseInt(start + (end - start) / 2, 10);
  if (end-start <= 1 || array[pivot] === element) return pivot;
  if (array[pivot] < element) {
    return locationOfElementInArray(element, array, pivot, end);
  } else {
    return locationOfElementInArray(element, array, start, pivot);
  }
}
...
function reduceTF(tf) { 
  ...
  for (var k in tf) {
    ...
    for (var i = 0; i < theseTokens.length; i++) {
      ...
      insertIntoOrderedArray(thisID, reducedTF[thisTFKey]);
    }
    reducedTF[tf[k]]
  }
  return reducedTF;
