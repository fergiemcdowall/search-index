...
//          console.log(field)
          ...
          var freq = tf.getTermFrequency(tv.getVector(field, {
            separator: batchOptions.separator,
            stopwords: batchOptions.stopwords,
...
//          console.log(facets)
          freq.forEach(function(item) {
            batchOptions.filters.forEach(function (filter) {
              _.forEach(doc[filter], function(filterKey){
                docIndexEntries.push({
                  type: 'put',
                  key: 'TF￮' + item[0] + '￮' + fieldName + '￮' + filter + '￮' + filterKey,
                  value: [doc.id]
                });
                docIndexEntries.push({
                  type: 'put',
                  key: 'RI￮' + item[0] + '￮' + fieldName + '￮' + filter + '￮' + filterKey,
                  value: [item]
                });
              })
            });
            docIndexEntries.push({
              type: 'put',
              key: 'TF￮' + item[0] + '￮' + fieldName + '￮￮',
              value: [doc.id]
            });
            docIndexEntries.push({
              type: 'put',
              key: 'RI￮' + item[0] + '￮' + fieldName + '￮￮',
              value: [item]
            });
          });
        })
        ...
        batchIndexEntries.push(docIndexEntries);
      });
      var reducedBatch = [];
      console.log(
        _(batchIndexEntries)
